...
from datetime import datetime as dm #imports datetime
                                    #from datetime
                                    #module as dm

import stat #imports stat

import shutil #imports shutil

import wx  #imports WxPython to
           #make an app and
           #find the monitor
           #size
...
import keyboard  #import the keyboard
                 #module, allowing
                 #the script to
                 #send keystrokes
...
userdic = {} #creates a dictionary for
             #user name and password

usernum = {} #and a dictionary for
             #username and user number
...
def deposit(user): #a function for the user
                   #to interact with and
                   #add money to their
                   #account

    balance=getbal(usernum[user]) #find the amount of
                                  #money in their
                                  #account
    ...
    if type(choice)==str: #if the input type
                          #entered is invalid
                          #and not "cancel"

        sprint("String input is not valid! An integer or float input is required. Please try again.") #tell the user that
                                                                                                      #they have entered
                                                                                                      #invalid input
        return deposit(user) #ask again
    if not choice%5 == 0 or not int(choice)>0: #if they have
                                               #entered a
                                               #float or integer,
                                               #but it is not
                                               #a valid cash
        ...
        sprint("This amount is invalid! Only combinations of cash notes can be accepted! Please try again.") #tell the user
                                                                                                             #that they have
                                                                                                             #entered invalid
                                                                                                             #input
    ...
    elif choice%5==0: #if the cash
                      #amount is a
                      #valid amount,
                      #continue on

        choice=int(str(choice).replace(".0", "")) #removes the decimal
                                                  #from the end

        getbal(usernum[user], "write", balance+int(choice)) #writes the new
                                                            #balance to the
                                                            #user's line in
                                                            #the balances file

        sprint(f"You deposit ${choice}.00 into account {correctcaps(user, ['all'])}. Your new balance is ${balance+choice}.00") #successful deposit
                                                                                                                                #output message
        ask(user, "deposit", choice) #function to ask
                                     #if the user wants
                                     #to see their
                                     #receipt for the
                                     #deposit
...
def withdraw(user): #function to deposit
    balance=getbal(usernum[user]) #find the amount
                                  #of money in
                                  #their account
    ...
    if type(choice)==str: #if the input
                          #type entered
                          #is invalid
                          #and not "cancel"

        sprint("String input is not valid! An integer or float input is required. Please try again.") #tell the user
                                                                                                      #that they have
                                                                                                      #entered invalid
                                                                                                      #input
    ...
    if not ((choice%10==0 and choice > 40) or choice%20==0) or not choice>0: #if they have
        ...
        sprint("This amount is invalid! Only combinations of $20 and $50 cash notes can be withdrawn! Please try again.") #tell user what
                                                                                                                          #input is valid

        return withdraw(user) #ask again

    if choice > balance: #checks for if
                         #the user is
                         #withdrawing
                         #too much

        sprint(f"This amount is invalid! You are trying to withdraw ${choice}, but you only have ${balance} in your account!") #tell the user
                                                                                                                               #that they do
                                                                                                                               #not have
                                                                                                                               #enough money
        ...
        choice=int(str(choice).replace(".0", "")) #remove the
                                                  #decimal from
                                                  #the end

        getbal(usernum[user], "write", balance-int(choice)) #write the new
                                                            #balance to the
                                                            #user's line
                                                            #in the balances
                                                            #file

        sprint(f"You withdraw ${choice}.00 from account {correctcaps(user, ['all'])}. Your new balance is ${balance-choice}.00") #successful withdrawl
                                                                                                                                 #output message
        ask(user, "withdraw", choice) #function to
                                      #ask if the user
                                      #wants to see
                                      #their receipt
                                      #for the
...
def receipt(user, transact, amount): #function to make a receipt
                                     #based on the input user,
                                     #process, and amount

    now = dm.now() #get a variable from
  ...
  {now.strftime('Great Bank® Ltd.  %Y')}''')
    ...
    while a==True: #until a is not true
        ...
        if not os.path.exists(f"receipts/{user}/"): #Checks for if
                                                    #a user folder
                                                    #exists

            os.mkdir(f"receipts/{user}/") #makes a directory for
                                          #the user if none exists

        for count in range(1, slots+1): #repeat three times,
                                        #for reciept 1,
                                        #receipt 2, and
                                        #receipt 3

            try: #attempt to make a
                 #new file with count
                 #as the receipt number

                receipt=open(f"receipts/{user}/receipt {count}.txt", "x") #test if receipt count
                                                                          #exists already, if it
                                                                          #does, throws exception

                receipt.write(transactlog) #if it does not, writes
                                           #the receipt contents
                                           #into it

                receipt.close() #closes the file

                a=False #makes a not true so
                        #that the function
                        #goes to end
                ...
                for line in open(f"receipts/{user}/receipt {count}.txt"): #opens the file
                                                                          #that is known
                                                                          #to exist

                    try: #for every line, checks
                         #if it has the date
                         #and time on it

                        dic[line.strip("Date and Time: ").split(" ")[2]]=count #if it is the correct line,
        ...
        if a==True: #will run if all
            ...
            for element in dic: #each time extracted
                                #corresponds to the
                                #receipt number it
                                #was extracted from

                lis.append(element) #make a list
                                    #from all the
                                    #keys

                lis=sorted(lis) #sort the list
                                #(to find
                                #the oldest one)

            count=dic[lis[0]] #sets receipt number
                              #to that of oldest
                              #receipt
...
def ask(user, transact, amount): #function to ask the user
    ...
    if print_receipt in ["y", "yes"]: #if they answer
        ...
        print("\nThe screen will clear in 10 seconds.") #tell user what is
                                                #happening with
                                                #wait

        t(10) #wait for 10 seconds,
              #or (due to interrupt
              #parameter) until
              #KeyboardInterrupt
              #is thrown
...
def logo(height_in, size="large", rtn="no"): #Defines the logo() function
    ...
    testforvalue("screen_height", 50) #set values for
                                      #screen_width
                                      #and screen_height
                                      #if none exist
                                      #already
    ...
    try: #attempt to
        ...
        screen_width=40 #set screen_width
                        #to 40 as a default

    global charbreak #charbreak for
                     #the sprint
                     #function

    olc=charbreak #preserve charbreak

    charbreak=0 #no pause
                #between
                #letter prints
    ...
    for i in range(int(screen_width/60)): #repeat for 1/60th
                                          #of the screen width

        print("") #add a spacer to top

    gap=int(screen_width/80)*" " #find the gap
                                 #from the side
                                 #of the screen
    ...
    for line in log.split("/n"): #print the logo
                                 #line by line?
        print(line)
    for j in range((screen_height-height_in)-20): #loop to space
                                                  #up the logo
...
def local(s_w=32, s_h=38): #local function, this
                           #reads between
                           #universal start
                           #function and the
                           #local functions,
    ...
    global screen_width #makes the variable
                        #screen_width Global

    global screen_height #makes the variable
    ...
    gap1=int((screen_width-128)/2)*" " #spacingfrom the
                                       #side of the
                                       #screen for
                                       #'Great'

    gap2=int((screen_width-96)/2)*" " #spacing from the
                                      #side of the
                                      #screen for
                                      #'Bank'
...
def register(name,password):
  ...
  file = open("users.txt","a") #a+ Opens a file for both
                               #appending and reading.
                               # The file pointer is at
                               #the end of the file if
                               # the file exists. The
                               #file opens in the append mode.
                               # If the file does not exist,
                               # it creates a new file for
                               #reading and writing

  global nextmark
  file.write(f'\n{name},{password},{nextmark}') #writes name and
                                                #password to
  ...
  file=open("balances.txt", "a") #open the balances
                                 #text file
  if int(nextmark)==70:
      amount="421.00"
...
def begin(something=""):  #the beginning
                          #of the whole
                          #login thing
...
def access(option, name=""):  #script to ask
                              #for username
                              #and password
    ...
    if option in ["login", "l"]:  #Checks what the
                                  #user has input
                                  #and runs the
                                  #next script
                                  #based on
...
############################################################################# Optimises user Experience in the command line
...
try:# all of this
    ...
    os.get_terminal_size()  #this functions
                            #throws an
                            #exception if
                            #the script is
                            #not running in
    ...
    def sprint(input_string, words_or_letters="letters", newline="yes"):  # this redefines
        # the sprint
        # function to be
        # terminal-friendly
    ...
    def checkintype(input_string, list_of_types, letter="", slow=True):  # repeat until
        ...
        if slow == True:
            ...
            iner = checktype(tinput(input_string, letter))  # make iner
            # a float or
            # int if possible
        ...
        for value in list_of_types:
            ...
            if type(iner) == value:  # if type of
                # input matches
                # requirement

                return iner  # return and
                # end strings

            stringlist.append(
                str(value).strip("<cla>").replace("s", "", 2).replace(" ", "").replace("'", ""))  # turn types into
        ...
        print(str(type(iner)).strip("<cla>").replace("s", "", 2).replace(" ", "").replace("'",
                                                                                          "") + " is incorrect type!")  # print fail
    ...
    def checkinput(input_string, comparison_list=["y", "yes", "n", "no"], modifier_list=[""]):  # asks for input
        # until given input
        # is in given
        # list of accepted
        ...
        if innn in comparison_list:  # if input is
            # in list given,
            # then return it
    ...
    try: #attempt to make
      ...
      os.makedirs(mypath,0o777) #makes a folder
                                #for the file

      open(fname,"w").close() #makes a marker
                              #file
